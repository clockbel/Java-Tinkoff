...
public class GammaCorrector implements ImageProcessor {
    private static final double GAMMA_FACTOR = 2.2;
        ...
        for (int x = 0; x < image.width(); ++x) {
            for (int y = 0; y < image.height(); ++y) {
                ...
                image.setPixel(new Pixel(
                    (int) (pixel.r() * Math.pow(normal, (1 / GAMMA_FACTOR))),
                    (int) (pixel.g() * Math.pow(normal, (1 / GAMMA_FACTOR))),
                    (int) (pixel.b() * Math.pow(normal, (1 / GAMMA_FACTOR))),
                    normal,
                    pixel.hitCount()
                ), x, y);
            }
